                       ...
                       HRATEEFF *rate_table, double fsR, double meR)
{
   ...
   for (l = 0; l <= 1; l++)
   {
      ...
      for (k = 0; k < 4; k++)
      {
         temp[k] = rate_table->logAlpha_tab[l][iTM - 1 + k][iTR - 1] * coeff2[0] + rate_table->logAlpha_tab[l][iTM - 1 + k][iTR] * coeff2[1] + rate_table->logAlpha_tab[l][iTM - 1 + k][iTR + 1] * coeff2[2] + rate_table->logAlpha_tab[l][iTM - 1 + k][iTR + 2] * coeff2[3];
      }
      ...
      Beta[l] = square(fsR / meR) * exp(rate_table->logAlpha_tab[l][NTM - 1][iTR - 1] * coeff2[0] + rate_table->logAlpha_tab[l][NTM - 1][iTR] * coeff2[1] + rate_table->logAlpha_tab[l][NTM - 1][iTR + 1] * coeff2[2] + rate_table->logAlpha_tab[l][NTM - 1][iTR + 2] * coeff2[3]);
   ...
   *R2p2s = fsR * fsR * fsR * fsR * fsR * meR *
            exp(rate_table->logR2p2s_tab[iTR - 1] * coeff2[0] + rate_table->logR2p2s_tab[iTR] * coeff2[1] + rate_table->logR2p2s_tab[iTR + 1] * coeff2[2] + rate_table->logR2p2s_tab[iTR + 2] * coeff2[3]);
}
...
#if (EFFECT_B == 0)
   for (b = 0; b < NSUBLYA; b++)
      twog->A3s3d_tab[b] = twog->A4s4d_tab[b] = 0;
#endif
#if (EFFECT_C == 0)
                        ...
                        double Alpha[2], double DAlpha[2], double Beta[2], double fsR, double meR)
{
   ...
   for (b = 0; b < NVIRT; b++)
   {
      ...
      Trr[1][1] -= Tvr[1][b] = -exp(-E32 / TR) / 3. * rescale2g * twog->A3s3d_tab[b] / fabs(exp((twog->Eb_tab[b] - E31) / TR) - 1.) - exp(-E42 / TR) / 3. * rescale2g * twog->A4s4d_tab[b] / fabs(exp((twog->Eb_tab[b] - E41) / TR) - 1.);
                     ...
                     double *Tvv[3], double sr[2], double sv[NVIRT])
{
   ...
   /** Allocate memory **/
   for (i = 0; i < 2; i++)
      Tvv_inv_Tvr[i] = create_1D_array(NVIRT);
                                  ...
                                  double zstart, unsigned iz, double z, double fsR, double meR)
{
   ...
   for (i = 0; i < 2; i++)
      Trv[i] = create_1D_array(NVIRT);
   for (i = 0; i < 2; i++)
      Tvr[i] = create_1D_array(NVIRT);
   for (i = 0; i < 3; i++)
      Tvv[i] = create_1D_array(NVIRT);
   ...
   dxedlna = -(nH * (s * x1s * DAlpha[0] + Alpha[0] * Dxe2) - xr[0] * Beta[0] + nH * (s * x1s * DAlpha[1] + Alpha[1] * Dxe2) - xr[1] * Beta[1]) / H;
