...
{ /* Rescaled two-photon decay rate */
...
void allocate_and_read_atomic(HYREC_ATOMIC *atomic, int *error, char *path_to_hyrec, char error_message[SIZE_ErrorM])
{
  ...
  L2s1s_current = 0.;
  for (b = 0; b < NSUBLYA; b++)
    L2s1s_current += atomic->A2s_tab[b];
  for (b = 0; b < NSUBLYA; b++)
    atomic->A2s_tab[b] *= L2s1s / L2s1s_current;
...
#if (EFFECT_B == 0)
  for (b = 0; b < NSUBLYA; b++)
    atomic->A3s3d_tab[b] = atomic->A4s4d_tab[b] = 0;
#endif
#if (EFFECT_C == 0)
  for (b = NSUBLYA; b < NVIRT; b++)
    atomic->A3s3d_tab[b] = atomic->A4s4d_tab[b] = 0;
#endif
#if (EFFECT_D == 0)
  for (b = NSUBLYA; b < NVIRT; b++)
    atomic->A2s_tab[b] = 0;
  for (b = NSUBLYB; b < NVIRT; b++)
    atomic->A3s3d_tab[b] = 0;
#endif
#if (DIFFUSION == 0)
  for (b = 0; b < NVIRT; b++)
    atomic->A1s_tab[b] = 0;
                       ...
                       HYREC_ATOMIC *atomic, double fsR, double meR, int *error, char error_message[SIZE_ErrorM])
{
  ...
  for (l = 0; l <= 1; l++)
  {
    ...
    Alpha_eq[l] = square(fsR / meR) * exp(atomic->logAlpha_tab[l][NTM - 1][iTR - 1] * coeff2[0] + atomic->logAlpha_tab[l][NTM - 1][iTR] * coeff2[1] + atomic->logAlpha_tab[l][NTM - 1][iTR + 1] * coeff2[2] + atomic->logAlpha_tab[l][NTM - 1][iTR + 2] * coeff2[3]);
  ...
  *R2p2s = fsR * fsR * fsR * fsR * fsR * meR *
           exp(atomic->logR2p2s_tab[iTR - 1] * coeff2[0] + atomic->logR2p2s_tab[iTR] * coeff2[1] + atomic->logR2p2s_tab[iTR + 1] * coeff2[2] + atomic->logR2p2s_tab[iTR + 2] * coeff2[3]);
  ...
  for (l = 0; l <= 1; l++)
  {
    ...
    for (k = 0; k < 4; k++)
    {
      temp[k] = atomic->logAlpha_tab[l + i][iTM - 1 + k][iTR - 1] * coeff2[0] + atomic->logAlpha_tab[l + i][iTM - 1 + k][iTR] * coeff2[1] + atomic->logAlpha_tab[l + i][iTM - 1 + k][iTR + 1] * coeff2[2] + atomic->logAlpha_tab[l + i][iTM - 1 + k][iTR + 2] * coeff2[3];
    }

    Alpha[l] = square(fsR / meR) * exp(temp[0] * coeff1[0] + temp[1] * coeff1[1] + temp[2] * coeff1[2] + temp[3] * coeff1[3]);
...
double rec_swift_hyrec_dxHIIdlna(HYREC_DATA *data, double xe, double xHII, double nH, double H, double TM, double TR, double z)
{
  ...
  if (TR / kBoltz > fit->swift_func[0][DKK_SIZE - 1])
    DK_K = 0.;
  else
  {
                        ...
                        double TM, double Eb_tab[NVIRT], double A1s_tab[NVIRT])
{
  ...
  for (b = NSUBLYA - NDIFF / 2 + 1; b < NSUBLYA - 1; b++)
  {
    ...
    Aup[b] = (DE2 * A1s_tab[b] - square(Eb_tab[b] - Eb_tab[b - 1]) * Adn[b]) / square(Eb_tab[b + 1] - Eb_tab[b]); /* Aup[b] , Adn[b] must add up to correct diffusion rate */
  }
  /* Last bin below Lyman alpha */
  b = NSUBLYA - 1;
  Adn[b] = exp((Eb_tab[b] - Eb_tab[b - 1]) / TM) * Aup[b - 1];

  Aup[b] = (DE2 * A1s_tab[b] - square(Eb_tab[b] - Eb_tab[b - 1]) * Adn[b]) / square(E21 - Eb_tab[b]);
  *A2p_dn = exp((E21 - Eb_tab[b]) / TM) / 3. * Aup[b]; /* 2p -> NSUBLYA-1 rate obtained by detailed balance */
  ...
  for (b = NSUBLYA + NDIFF / 2 - 2; b > NSUBLYA; b--)
  {
    ...
    Adn[b] = (DE2 * A1s_tab[b] - square(Eb_tab[b + 1] - Eb_tab[b]) * Aup[b]) / square(Eb_tab[b] - Eb_tab[b - 1]);
  }
  /* First bin above Lyman alpha */
  b = NSUBLYA;
  Aup[b] = exp((Eb_tab[b] - Eb_tab[b + 1]) / TM) * Adn[b + 1];

  Adn[b] = (DE2 * A1s_tab[b] - square(Eb_tab[b + 1] - Eb_tab[b]) * Aup[b]) / square(Eb_tab[b] - E21);
                        ...
                        int *error, char error_message[SIZE_ErrorM])
{
  ...
  Trr[0][0] = Beta[0] + 3. * R2p2s + 3. * RLya * (1.664786871919931 * exp(-E32 / TR) /* Ly-beta escape */
  ...
  for (b = 0; b < NVIRT; b++)
  {
    ...
    Trr[1][1] -= Tvr[1][b] = -exp(-E32 / TR) / 3. * rescale2g * atomic->A3s3d_tab[b] / fabs(exp((atomic->Eb_tab[b] - E31) / TR) - 1.) - exp(-E42 / TR) / 3. * rescale2g * atomic->A4s4d_tab[b] / fabs(exp((atomic->Eb_tab[b] - E41) / TR) - 1.);
                     ...
                     double sr[2], double sv[NVIRT], int *error, char error_message[SIZE_ErrorM])
{
  ...
  /** Allocate memory **/
  for (i = 0; i < 2; i++)
    Tvv_inv_Tvr[i] = create_1D_array(NVIRT, error, error_message);
                                  ...
                                  unsigned iz, double z)
{
  ...
  for (i = 0; i < 2; i++)
    Trv[i] = create_1D_array(NVIRT, error, data->error_message);
  for (i = 0; i < 2; i++)
    Tvr[i] = create_1D_array(NVIRT, error, data->error_message);
  for (i = 0; i < 3; i++)
    Tvv[i] = create_1D_array(NVIRT, error, data->error_message);
  ...
  dxedlna = -(nH * (s * x1s * DAlpha[0] + Alpha[0] * Dxe2) - xr[0] * Beta[0] + nH * (s * x1s * DAlpha[1] + Alpha[1] * Dxe2) - xr[1] * Beta[1]) / H + cosmo->inj_params->ion / H; /* First term automatically includes the additional excitations
  ...
  for (b = 0; b < NVIRT; b++)
  {
    if (Dtau[b] > 1e-30)
    {
      ...
      Dfeq -= (b == 0 ? xv[1] * Tvv[2][0] : b == NVIRT - 1 ? xv[NVIRT - 2] * Tvv[1][NVIRT - 1]
                     ...
                     unsigned iz, double z)
{
  ...
  if (*error == 1)
    return 0.;
  if (model == PEEBLES)
    result = rec_TLA_dxHIIdlna(cosmo, xe, xHII, nH, H, TM, TR, 1.00);
  else if (model == RECFAST)
    result = rec_TLA_dxHIIdlna(cosmo, xe, xHII, nH, H, TM, TR, 1.14);
  else if (model == EMLA2s2p)
    result = rec_HMLA_dxHIIdlna(data, xe, xHII, nH, H, TM, TR);
  else if (model == FULL || model == SWIFT)
  {
    ...
    {
      if (model == FULL)
        result = rec_HMLA_2photon_dxHIIdlna(data, xe, xHII, nH, H, TM, TR, iz, z);
      else if (model == SWIFT)
      {
        if (TR / kBoltz / cosmo->fsR / cosmo->fsR / cosmo->meR < fit->swift_func[0][0])
          result = rec_HMLA_dxHIIdlna(data, xe, xHII, nH, H, TM, TR);
        else
          result = rec_swift_hyrec_dxHIIdlna(data, xe, xHII, nH, H, TM, TR, z);
      }
    }
  }
