                             ...
                             const double *yp, const double *num_nu, double *xe, double *Tm, long int *nz, const double *pann)
{
  ...
  if (MODEL == SWIFT)
    rec_data.cosmo->dlna = DLNA_SWIFT;
  else
    rec_data.cosmo->dlna = DLNA_HYREC;
  ...
  rec_data.xe_output = xe;
  rec_data.Tm_output = Tm;
...
double rec_HubbleRate(REC_COSMOPARAMS *cosmo, double z)
{
  ...
  else
  {
    ...
    for (i = 0; i < cosmo->Nmnu; i++)
    {
      ...
      cosmo->onuh2 = cosmo->onuh2 + (1. + 0.317322 * 0.0457584 * pow(y, 3.47446 + 1.) + 2.05298 * 0.0457584 * pow(y, 3.47446 - 1.)) / (1. + 0.0457584 * pow(y, 3.47446)) * (3.45e-8 * Tnu0 * Tnu0 * Tnu0 * Tnu0) * 5.6822 * 2;
    }
  }
...
void rec_get_cosmoparam(FILE *fin, FILE *fout, REC_COSMOPARAMS *param)
{
  ...
  {
    if (fout != NULL)
      fprintf(fout, "Error in rec_get_cosmoparam when reading parameter 'meR'\n");
    exit(1);
  };
  ...
  if (param->Nmnu != 0)
  {
    for (i = 0; i < param->Nmnu; i++)
    {
      ...
      param->onuh2 = param->onuh2 + (1. + 0.317322 * 0.0457584 * pow(y, 3.47446 + 1.) + 2.05298 * 0.0457584 * pow(y, 3.47446 - 1.)) / (1. + 0.0457584 * pow(y, 3.47446)) * (3.450e-8 * Tnu0 * Tnu0 * Tnu0 * Tnu0) * 5.6822 * 2;
    }
  }
...
double hyrec_integrator(double deriv, double deriv_prev[2], double z)
{
  ...
  if (MODEL == FULL)
    ...
    result = 23. / 12. * deriv - 16. / 12. * deriv_prev[0] + 5. / 12. * deriv_prev[1];
                         ...
                         double *hubble_array, int flag)
{
  ...
  if (hubble_array[0] == -1.)
    ...
    H = rec_interp1d(.0, dz, hubble_array, Nz, z_in, error, sub_message);
  ...
  if (data->quasi_eq == 1)
  {
    ...
    dxHeIISaha_dlna = (1. + z_out) * (rec_saha_xHeII(cosmo, z_out - 0.5) - rec_saha_xHeII(cosmo, z_out + 0.5));
    ...
    if (hubble_array[0] == -1.)
      ...
      H = rec_interp1d(.0, dz, hubble_array, Nz, z_out, error, sub_message);

    DdxHeIIdlna_Dxe = (rec_helium_dxHeIIdlna(data, z_out, xH1_p, xHeIISaha + Dxe, H) - rec_helium_dxHeIIdlna(data, z_out, xH1_m, xHeIISaha - Dxe, H)) / 2. / Dxe;
...
void rec_xH1_stiff(HYREC_DATA *data, int model, double z, double xHeII, double *xH1, unsigned iz_rad, double H)
{
  ...
  else
  {
    ...
    if (model == SWIFT)
    {
      if (T / kBoltz / cosmo->fsR / cosmo->fsR / cosmo->meR > data->fit->swift_func[0][DKK_SIZE - 1])
        model_stiff = EMLA2s2p;
    }
  }
  ...
  dxH1sSaha_dlna = (1. + z) * (rec_saha_xH1s(cosmo, z - 0.5, xHeII) - rec_saha_xH1s(cosmo, z + 0.5, xHeII));
  /* (partial xHII)/(partial lna). Use xH1s = 1-xHII for better accuracy. */
  if (xHeII != 0.)
  {
    dxH1sSaha_dlna = (1. + z) * (rec_saha_xH1s(cosmo, z - 0.5, xHeII) - rec_saha_xH1s(cosmo, z + 0.5, xHeII));
    Dxe = 0.01 * (cosmo->fHe - xHeII);
    dxH1sSaha_dlna += (rec_saha_xH1s(cosmo, z, xHeII + Dxe) - rec_saha_xH1s(cosmo, z, xHeII - Dxe)) / 2. / Dxe * rec_helium_dxHeIIdlna(data, z, xH1sSaha, xHeII, H);
  ...
  Dxe = 0.01 * xH1sSaha;
  DdxH1sdlna_DxH1s = (rec_dxHIIdlna(data, model_stiff, xHIISaha + Dxe + xHeII, xHIISaha + Dxe, nH, H, T, T, iz_rad, z) - rec_dxHIIdlna(data, model_stiff, xHIISaha - Dxe + xHeII, xHIISaha - Dxe, nH, H, T, T, iz_rad, z)) / 2. / Dxe;
  *xH1 = xH1sSaha + (dxH1sSaha_dlna - dxH1sdlna_Saha) / DdxH1sdlna_DxH1s;

  if (fabs(*xH1 - xH1sSaha) > DXHII_MAX)
    data->quasi_eq = 0;
  // if (z<1700.) *stiff = 0;   /* Used when calculating the correction function for SWIFT mode. */

  /* Update photon population when MODEL = FULL */
  if (model == FULL)
    rec_dxHIIdlna(data, model, xHeII + 1. - *xH1, 1. - *xH1, nH, H, T, T, iz_rad, z);
                          ...
                          double dTmdlna_prev[2], double H, double z_out, double H_next)
{
  ...
  if (TR / cosmo->fsR / cosmo->fsR / cosmo->meR <= TR_MIN || kBoltz * Tm_in / TR <= T_RATIO_MIN)
    model = PEEBLES;
  dxedlna = rec_dxHIIdlna(data, model, xe_in, xe_in, nH, H, kBoltz * Tm_in, TR, iz_rad, z_in);
  ...
  ********/
  ...
  for (; iz <= data->rad->iz_rad_0; iz++)
  {
    ...
    if (model == SWIFT && data->quasi_eq == 0 && data->loop_after_quasi == 1)
    {
      ...
      if (fabs(1 - dxHeIIdlna_prev[1] / dxHeIIdlna_prev[0]) < DXHEII_DIFF_MAX)
...
void hyrec_allocate(HYREC_DATA *data, double zmax, double zmin)
{
  ...
  if (MODEL == FULL)
    allocate_radiation(data->rad, (long int)(log(10.) / DLNA), &data->error, data->error_message);
