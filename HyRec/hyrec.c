...
int main(void)
{
   ...
   for (iz = 0; iz < NSPEC; iz++)
      z_spec[iz] = ZMIN_SPEC + (ZMAX_SPEC - ZMIN_SPEC) / (NSPEC - 1) * iz;        /* Redshifts at which spectrum is printed */
   ...
   for (iz = 0; iz < NSPEC; iz++)
      fprintf(fp, " %E", z_spec[iz]);
   fprintf(fp, "\n");

   for (b = 0; b < NSUBLYA; b++)
   { /* Sub-Lyman alpha bins */
   ...
   fprintf(fp, "\n");
   for (; b < NSUBLYB; b++)
   { /* Bins between Ly-alpha and Ly-beta */
      fprintf(fp, "%E", twog_params.Eb_tab[b] / E21);
      for (iz = 0; iz < NSPEC; iz++)
      {
         Dfnu = interp_Dfnu(-log(1. + param.zH0), DLNA, Dfnu_hist[b], izmax_spec, -log(1. + z_spec[iz]));
   ...
   fprintf(fp, "\n");
   for (; b < NVIRT; b++)
   { /* Bins between Ly-beta and Ly-gamma */
      fprintf(fp, "%E", twog_params.Eb_tab[b] / E21);
      for (iz = 0; iz < NSPEC; iz++)
      {
         Dfnu = interp_Dfnu(-log(1. + param.zH0), DLNA, Dfnu_hist[b], izmax_spec, -log(1. + z_spec[iz]));
